...
export const refresh = internalMutation({
    args:{
        contactSeesionId : v.id("contactSessions"),
    },
    handler: async(ctx, args) => {
        const contactSession = await ctx.db.get(args.contactSeesionId);

        if(!contactSession){
            throw new ConvexError({
                code: "NOT_FOUND",
                message: "Conttact Session Not Found",
            });
        };
        ...
        if(timeRemaining < AUTO_REFRESH_THRESHOLD_MS){
            ...
            await ctx.db.patch(args.contactSeesionId,{
